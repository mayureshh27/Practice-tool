...
function App(){
  ...
  const filtered=useMemo(()=>store.problems.filter(p=>(chapter==='all'||p.chapter===chapter)&&(mode==='all'||p.exerciseMode===mode)&&((p.title+p.tags.join(' ')+p.statement+(p.problemText||'')+(p.explanation||'')+(p.howItWorks||'')+(p.syntax||'')+(p.solve||'')).toLowerCase().includes(query.toLowerCase()))),[store,query,chapter,mode]);
  function select(p:Problem){setPid(p.id); setCode(localStorage.getItem('draft:'+p.id)||p.starterCode); setUploaded(JSON.parse(localStorage.getItem('files:'+p.id)||'[]')); setOut(''); setVerdict(''); setTab('explanation')}
